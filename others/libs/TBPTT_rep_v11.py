...
class TBPTT_rep_trainer:
    ...
    def process_epoch(self, data, epoch, num_epochs, data_max, data_min, training=True):
        ...
        # mask確率
        p_seq = 0
        # p_seq = min(0.3, (epoch + 1) / max(1, num_epochs))
        ...
        for n_batch, ((x_img, x_joint), (y_img, y_joint)) in enumerate(data):
            ...
            for t in range(total_steps):
                ...
                if is_chunk_end or is_last_step:
                    ...
                    pos_loss = (pos_weights * (delta_pos_hat_mm - delta_pos_gt_mm).pow(2)).mean()
                    grip_loss = F.mse_loss(grip_hat, grip_gt)

                    joint_loss = (pos_loss + grip_loss*5e-2) * self.scheduler_pos(
                        self.loss_weights[1]
                    )  # 変更: jointにもスケジューリングを入れる
                    # joint_loss = (pos_loss + grip_loss) * self.loss_weights[1]
                    ...
                    with torch.no_grad():
                        ...
                        diff = delta_pos_hat_mm - delta_pos_gt_mm  # (B, T, 3)
