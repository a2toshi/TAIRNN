...
from libs.convTBPTT_rep_v4 import TBPTT_rep_trainer  # TODO
from ModelCode.data import MultimodalDataset  # TODO
from ModelCode.model import ConvTAiRNNv8, ConvTAiRNNv9  # TODO
...
epoch_ckpt_dir = os.path.join(log_dir_path, "epoch_checkpoints")
...
with open(output_path, "w") as file:
    ...
    file.write("2025/11/19: train_TAiRNN_conv_wo_v5")
...
optimizer = optim.Adam(model.parameters(), eps=1e-07, lr=args.lr, weight_decay=1e-4)
...
trainer = TBPTT_rep_trainer(
    model,
    optim.Adam,
    loss_weights=loss_weights,
    joint_dim=args.joint_dim,
    truncation_length=args.chunk_size,
    device=device,
)
scheduler = CosineAnnealingLR(trainer.optimizer, T_max=args.epoch, eta_min=args.lr * 0.01)
### training main
...
for epoch in pbar_epoch:
    ...
    (
        ...
        train_pos_y_loss,
        train_pos_z_loss,
    ) = trainer.process_epoch(train_loader, epoch=epoch, num_epochs=args.epoch, data_max=data_max, data_min=data_min)
    ...
    with torch.no_grad():
        (
            ...
            test_loader, epoch=epoch, num_epochs=args.epoch, data_max=data_max, data_min=data_min, training=False
        )
    ...
    writer.add_scalar("Loss/posx_train_loss", train_pos_x_loss, epoch)
    writer.add_scalar("Loss/posy_train_loss", train_pos_y_loss, epoch)
    writer.add_scalar("Loss/posz_train_loss", train_pos_z_loss, epoch)
    ...
    writer.add_scalar("Loss/posx_test_loss", test_pos_x_loss, epoch)
    writer.add_scalar("Loss/posy_test_loss", test_pos_y_loss, epoch)
    writer.add_scalar("Loss/posz_test_loss", test_pos_z_loss, epoch)
    ...
    if (epoch + 1) % 100 == 0:
        ckpt_path = os.path.join(epoch_ckpt_dir, f"epoch_{epoch+1:05d}.pth")
        trainer.save(epoch, [train_loss, test_loss], ckpt_path, scheduler.state_dict())
