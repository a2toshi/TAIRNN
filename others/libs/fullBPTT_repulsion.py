...
class fullBPTT_rep_trainer:
    ...
    def process_epoch(self, data, training=True):
        ...
        for n_batch, ((x_img, x_joint), (y_img, y_joint)) in enumerate(data):
            ...
            # 損失計算
            y_joint_sliced = y_joint[:, :, :8]  # カメラ座標系座標(7,8,9)を取り除く
            ...
            # pt_loss = nn.MSELoss()(torch.stack(dec_pts_list[:-1]), torch.stack(enc_pts_list[1:])) * self.loss_weights[2]

            # rep_losses = []
            # for t in range(len(enc_pts_list)):
            #     Penc = enc_pts_list[t].view(-1, self.model.k_dim, 2)
            #     Pdec = dec_pts_list[t].view(-1, self.model.k_dim, 2)
            #     rep_losses.append(repulsion_loss(Penc, margin=0.08))
            #     rep_losses.append(repulsion_loss(Pdec, margin=0.08))
            # rp_loss = torch.stack(rep_losses).mean() * self.scheduler(self.loss_weights[3])  # 新しい重み

            # 追加
            # A = torch.stack(attn_maps_list, dim=1)  # (B, T, K', N)
            # A = A.reshape(-1, A.shape[2], A.shape[3])  # (B*T, K', N)
            # rp_raw = peak_weighted_ortho_loss(A, gamma=1.5)
