...
# load dataset
root_dir = "/work/gn45/n45002/savedata/traindata_20250621_v2/"
# root_dir = '/work/gn45/n45002/savedata/savedata_test/'
testdata_folder = sorted([name for name in os.listdir(root_dir)])
testdata_folder = [testdata_folder[13]]
print(testdata_folder)


# dataset_loaderの呼び出しを修正
images, joints, _, _ = dataset_loader(testdata_folder, root_dir, joint_dim=7, data_min=data_min, data_max=data_max)
images = images[0]
joints = joints[0]
...
# 新しいモデルの設計図に合わせた、空のstate_dictを作成
new_state_dict = model.state_dict()

# 古いstate_dictから、名前を変更しながら新しいstate_dictに重みをコピー
for key, value in original_state_dict.items():
    # 'pos_encoder.' で始まるキーを、新しい名前に変更する
    if key.startswith("pos_encoder."):
        # 例: "pos_encoder.0.weight" -> "pos_encoder_cnn.0.weight"
        #     "pos_encoder.6.pos_x"  -> "pos_encoder_softmax.pos_x"

        # Sequentialのインデックスを取得 (例: "0", "2", "4", "6")
        layer_index = int(key.split(".")[1])

        # インデックスが6未満ならCNN部分、6ならSoftmax部分
        if layer_index < 6:
            # 新しいキー名を作成
            new_key = key.replace("pos_encoder", "pos_encoder_cnn")
            new_state_dict[new_key] = value
        else:
            # pos_encoder.6.pos_x -> pos_encoder_softmax.pos_x のように変換
            # まず "pos_encoder.6." の部分を "pos_encoder_softmax." に置換
            new_key = key.replace(f"pos_encoder.{layer_index}.", "pos_encoder_softmax.")
            new_state_dict[new_key] = value

    # pos_encoder以外のキーは、名前が一致していればそのままコピー
    elif key in new_state_dict:
        new_state_dict[key] = value

# 変更した新しいstate_dictをモデルにロードする
