...
# 変更: convv6からtokとAttention Mapを掛け合わせたものを入力するように変更
...
class ConvTAiRNNv7(nn.Module):
    ...
    def __init__(
        ...
        super(ConvTAiRNNv7, self).__init__()
        # print("メモ")
        ...
        small_tok_dim = 8
        self.tok_compressor = nn.Conv2d(attn_dim, small_tok_dim, kernel_size=1)

        # RNN
        conv_input_dim = self.joint_dim + small_tok_dim * self.k_dim
    ...
    def forward(self, xi, xv, state=None):
        ...
        attn_gate = F.softmax(torch.log(processed_attn_w + eps) / self.temp_sche, dim=-1)
        B, K_heads, N = attn_gate.shape
        attn_gate = attn_gate.reshape(B, K_heads, self.Hp, self.Wp)
        tok_small = self.tok_compressor(tok)
        gated_features = attn_gate.unsqueeze(2) * tok_small.unsqueeze(1)
        B, K, C_small, H, W = gated_features.shape
        gated_input = gated_features.reshape(B, K * C_small, H, W)

        # 5) RNN ―――
        B, _, Hp, Wp = tok.shape
        xv_map = xv_joint.view(B, self.joint_dim, 1, 1).expand(-1, -1, Hp, Wp)
        lstm_in = torch.cat([xv_map, gated_input], dim=1)
