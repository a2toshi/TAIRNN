...
class TBPTT_rep_trainer:
    ...
    def process_epoch(self, data, epoch, num_epochs, training=True):
        ...
        p_seq = min(0.7, (epoch + 1) / max(1, num_epochs) * 0.7)
        ...
        for n_batch, ((x_img, x_joint), (y_img, y_joint)) in enumerate(data):
            ...
            for t in range(total_steps):
                ...
                pos_info = current_x_joint[:, :3]
                pos_info = current_x_joint[:, :3]  # (B,3)

                masked_pos_info = torch.where(
                    isMasked, torch.full_like(pos_info, 0.5), pos_info  # (B,1) → (B,3) にブロードキャスト
                )
                masked_x_joint = torch.cat([masked_pos_info, current_x_joint[:, 3:]], dim=1)
                ...
                if is_chunk_end or is_last_step:
                    ...
                    y_img_chunk = y_img[:, start_t_in_chunk + 2 : t + 3]
                    y_joint_chunk = y_joint[:, start_t_in_chunk + 2 : t + 3]
                    y_joint_chunk_old = y_joint[:, start_t_in_chunk + 1 : t + 2]
