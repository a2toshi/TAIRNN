...
class ConvTAiRNNv4(nn.Module):
    ...
    def __init__(
        ...
        super(ConvTAiRNNv4, self).__init__()
        # print("memo")
    ...
    def forward(self, xi, xv, state=None):
        ...
        # lstm_in = torch.cat([attn_map, tok, torch.zeros_like(xv_map)], dim=1) #アブレーション用
        ...
        if self.visualize_on_forward:
            self.visualize_attn_maps(
                ...
                pred_attn_map=pred_attn_map,  # (B,K,Hp,Wp)
                save_np=False,
                save_png=True,
