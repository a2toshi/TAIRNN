...
class TAiRNNold(nn.Module):
    ...
    def forward(self, xi, xv, state=None):
        ...
        y_q = self.decoder_q(h)
        y_q = y_q / (torch.norm(y_q, dim=-1, keepdim=True) + 1e-6)

        logit_g = self.decoder_g(h)  # (B, 1)
        y_g = torch.sigmoid(logit_g)

        y_joint = torch.cat([y_pos, y_q, y_g], dim=1)
