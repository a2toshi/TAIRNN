...
import numpy as np
import torch
from dataset_loader_wo_quat import dataset_loader
from ModelCode.model import ConvTAiRNNv2, TAiRNNv5, TAiRNNv7, TAiRNNv8, ConvTAiRNNv5, ConvTAiRNNv6
from ModelCode.utils import deprocess_img, restore_args, tensor2numpy

# argument parser
parser = argparse.ArgumentParser()
parser.add_argument("--filename", type=str, default=None)
parser.add_argument("--ckpt", type=str, default="SARNN_latest.pth")
...
# load dataset
# root_dir = "/work/gn45/n45002/savedata/testdata_20250930_v2/"
root_dir = "/work/gn45/n45002/savedata/testdata_20251213_v1/"
...
test_folder_name = testdata_folder[0]
...
model = ConvTAiRNNv5(
    ...
    folder_name=f"{ckpt_stem}/{test_folder_name}",
...
# ベース: ./output/<filename>/<ckpt名>/<testフォルダ>/
base_out_dir = Path("./output") / args.filename / ckpt_stem / test_folder_name
joint_err_dir = base_out_dir / "joint_error/"
joint_err_dir.mkdir(parents=True, exist_ok=True)

log_path = joint_err_dir / "joint_error.txt"
log_fp = open(log_path, "w", encoding="utf-8")


def log_print(msg: str):
    """print しつつ joint_error.txt にも書く"""
    print(msg)
...
for loop_ct in range(nloop):
    ...
    pos_next_norm = y_joint[:, :3]
    grip_next_norm = y_joint[:, 3:4]
    joint_next_norm = torch.cat([pos_next_norm, grip_next_norm], dim=1)

    # denormalization
    pred_image = tensor2numpy(y_image[0])
    pred_image = deprocess_img(pred_image, 0, 1)
    pred_image = pred_image.transpose(1, 2, 0)
    pred_joint = tensor2numpy(joint_next_norm[0])
    ...
    log_print("loop_ct:{}, joint:{}".format(loop_ct, pred_joint))
...
log_print("===== Joint prediction error statistics (MODEL, per dimension) =====")
for d in range(joint_dim):
    log_print(f"[dim {d}] " f"bias={bias[d]:.6f}, " f"std={std[d]:.6f}, " f"MAE={mae[d]:.6f}, " f"RMSE={rmse[d]:.6f}")
...
log_print("===== Baseline (Δpos = 0) error statistics (per dimension) =====")
for d in range(joint_dim):
    log_print(
        f"[dim {d}] "
...
# （任意）改善度も見たいなら：
log_print("===== Improvement over baseline (RMSE_base - RMSE_model) =====")
for d in range(joint_dim):
    log_print(f"[dim {d}] ΔRMSE = {base_rmse[d] - rmse[d]:.6f}")
...
folder_name = str(joint_err_dir)  # 以降の既存コードが folder_name を使っているなら
...
for d in range(joint_dim):
    ...
    log_print(f"Saved joint error plot for dim {d} -> {save_path}")
