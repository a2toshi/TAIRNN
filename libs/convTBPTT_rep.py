...
def weighted_map_loss(pred_map, target_map, weight_peak=20.0):
    """
    Attentionが高い場所を「えこひいき」して学習させるMSE

    Args:
        pred_map: 予測されたMap (Sigmoid通した後推奨)
        target_map: 正解のMap (EncoderのAttention)
        weight_peak: 高いところをどれくらい重要視するか（例: 20倍）
    """
    # 1. 誤差の二乗 (通常のMSE)
    diff_sq = (pred_map - target_map) ** 2

    # 2. 重み係数を作る
    # 正解(target)の値が大きい場所ほど、重みが大きくなるようにする
    # 背景(0)の部分は重み1.0、ピーク(1.0)の部分は重み (1.0 + 20.0) = 21.0
    weights = 1.0 + weight_peak * target_map
