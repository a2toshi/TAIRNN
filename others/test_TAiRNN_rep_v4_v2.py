...
from ModelCode.model import TAiRNNv2
...
state = None
nloop = len(images)
joint_t = torch.Tensor(np.expand_dims(joints[0], 0))
for loop_ct in range(nloop):
    ...
    joint_t = torch.Tensor(np.expand_dims(tensor2numpy(y_joint[0]), 0))
...
gt_eval = true_joints[1 : 1 + eval_T, :joint_dim]  # (eval_T, joint_dim)
pred_eval = pred_joint[:eval_T, :joint_dim]  # (eval_T, joint_dim)
...
print("===== Joint prediction error statistics (per dimension) =====")
