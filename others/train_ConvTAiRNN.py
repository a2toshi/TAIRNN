...
from libs.ConvfullBPTT import ConvfullBPTTtrainer  # TODO
...
parser = argparse.ArgumentParser(description="TAiRNN")
...
# データセット
dataset_root_dir = "/work/gn45/n45002/savedata/traindata_20250713_comb/"
...
train_folders, test_folders = train_test_split(selected_folders, random_state=42, test_size=0.2)

images, joints, data_min, data_max = dataset_loader(
    train_folders, dataset_root_dir, joint_dim=args.joint_dim, img_size=args.im_size
)
...
with open(output_path, "w") as file:
    ...
    file.write("2025/08/18: img_lossを大きく & エントロピー最小化損失項を導入")
...
ModelClass = load_model(model_name="TAiRNN", class_name="TAiRNN")
model = ConvTAiRNN(
...
trainer = ConvfullBPTTtrainer(model, optimizer, loss_weights=loss_weights, device=device, joint_dim=args.joint_dim)
