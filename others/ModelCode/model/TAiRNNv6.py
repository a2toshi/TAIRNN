...
# 変更: token encoderあたりを変更
# 変更：位置エンコーディングのstdを0.001から0.02に変更
# 変更(1203): gのdecoderを削った
# 変更(1203): QにかかっているLNを外した
...
class LearnablePositionalEncoding(nn.Module):
    ...
    def init_weights(self):
        nn.init.normal_(self.pos_embed, std=0.001)  # 変更
...
class TAiRNNv6(nn.Module):
    ...
    def __init__(
        ...
        self.token_encoder = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=kernel_size, stride=1, padding=0),
            nn.BatchNorm2d(32),
            activation,
            nn.Conv2d(32, 64, kernel_size=kernel_size, stride=1, padding=0),
            nn.BatchNorm2d(64),
            activation,
            nn.Conv2d(64, attn_dim, kernel_size=kernel_size, stride=1, padding=0),
        ...
        # 2. 状態コンテキスト用プロジェクション
        self.state_proj = nn.Linear(self.joint_dim, self.attn_dim, bias=False)

        self.q_from_concat = nn.Linear(self.attn_dim * 2, self.attn_dim)
        ...
        self.decoder_pos = nn.Sequential(
            ...
            activation,
            nn.Dropout(0.1),
            nn.Linear(rec_dim // 4, 3),
        )
        self.decoder_g = nn.Sequential(
            nn.Linear(rec_dim, rec_dim // 4),
            activation,
            nn.Dropout(0.1),
        ...
        # 追加
        nn.init.constant_(self.state_proj.weight, 0.0)
        if self.state_proj.bias is not None:
            nn.init.constant_(self.state_proj.bias, 0.0)
    ...
    def forward(self, xi, xv, state=None):
        ...
        # 2) ロボット状態 → Query
        state_vec = torch.cat([xv_extra, xv_joint[:, 3:]], dim=1)
        state_embed = self.state_proj(state_vec)  # (B, attn_dim)
        state_embed = state_embed.unsqueeze(1).expand(-1, self.k_dim, -1)  # (B, k_dim, attn_dim)

        Q_input = torch.cat([self.object_queries.expand(B, -1, -1), state_embed], dim=-1)
        Q = self.q_from_concat(Q_input)  # Linear(2*attn_dim -> attn_dim)
        # Q = self.ln_q(Q)
        ...
        # 状態推定
        y_pos = self.decoder_pos(h)
        logit_g = self.decoder_g(h)  # (B, 1)
        y_g = torch.sigmoid(logit_g)
        y_joint = torch.cat([y_pos, y_g], dim=1)
