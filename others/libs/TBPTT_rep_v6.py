...
class TBPTT_rep_trainer:
    ...
    def process_epoch(self, data, epoch, num_epochs, data_max, data_min, training=True):
        ...
        # mask確率
        p_seq = 0.3
        sigma_pos = 0.05  # pos 用ノイズ標準偏差（正規化スケールで）
        sigma_quat = 0.0  # quat はノイズなし
        sigma_g = 0.05  # grip は少し大きめでもよい
        sigma_vec = torch.tensor(
            [sigma_pos, sigma_pos, sigma_pos, sigma_quat, sigma_quat, sigma_quat, sigma_quat, sigma_g, 0, 0, 0],
            device=self.device,
        ).view(
            1, -1
        )  # (1, joint_dim)
        ...
        for n_batch, ((x_img, x_joint), (y_img, y_joint)) in enumerate(data):
            ...
            isNoisy = torch.rand(batch_size, 1, device=self.device) < p_seq  # (B,1) True のシーケンスにノイズ

            for t in range(total_steps):
                current_x_joint = x_joint[:, t]

                noise = torch.randn_like(current_x_joint) * sigma_vec  # (B, joint_dim)
                noisy_x_joint = current_x_joint + noise * isNoisy.float()
                noisy_x_joint = torch.clamp(noisy_x_joint, 0.0, 1.0)

                _yi_hat, _yv_hat, enc_ij, dec_ij, state = self.model(x_img[:, t], noisy_x_joint, state)
