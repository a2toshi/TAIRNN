...
class ConvTAiRNNv6(nn.Module):
    ...
    def __init__(
        ...
        super(ConvTAiRNNv6, self).__init__()
        # print("メモ")
        ...
        # RNN
        conv_input_dim = self.joint_dim + self.k_dim
    ...
    def forward(self, xi, xv, state=None):
        ...
        lstm_in = torch.cat([attn_map, xv_map], dim=1)
        # lstm_in = torch.cat([torch.zeros_like(attn_map), xv_map], dim=1) #アブレーション用
