...
import matplotlib.pyplot as plt  # 【変更箇所】可視化のために追加
import numpy as np  # 【変更箇所】可視化のために追加
import torch
import torch.nn as nn
from ModelCode.layer import InverseSpatialSoftmax, SpatialSoftmax
...
class SARNN(nn.Module):
    ...
    def __init__(
        ...
        self.pos_encoder_cnn = nn.Sequential(
            nn.Conv2d(3, 16, 3, 1, 0),
            activation,
            nn.Conv2d(16, 32, 3, 1, 0),
            activation,
            nn.Conv2d(32, self.k_dim, 3, 1, 0),
            activation,
        )
        self.pos_encoder_softmax = SpatialSoftmax(
            width=sub_im_size[0],
            height=sub_im_size[1],
            temperature=self.temperature,
            normalized=True,
    ...
    def _save_attention_map(self, attn_map_tensor, save_dir):
        """
        Attention Mapを連番ファイルとしてヒートマップ画像で保存する。
        """
        if save_dir is None:
            return
        ...
        # ファイル名をカウンターを使って生成
        save_path = os.path.join(save_dir, f"attn_map_{self.vis_counter:04d}.png")

        attn_map_np = attn_map_tensor.detach().cpu().numpy()
        attn_map_to_vis = attn_map_np[0]

        k_dim, Hp, Wp = attn_map_to_vis.shape

        fig, axes = plt.subplots(1, k_dim, figsize=(k_dim * 3, 3))
        if k_dim == 1:
            axes = [axes]

        for i in range(k_dim):
            ax = axes[i]
            im = ax.imshow(attn_map_to_vis[i], cmap="jet")
    ...
    def forward(self, xi, xv, state=None):
        ...
        attn_map = self.pos_encoder_cnn(xi)
        enc_pts, _ = self.pos_encoder_softmax(attn_map)

        # save_attention_dir = "./output/"
        # if save_attention_dir:
        #     self._save_attention_map(attn_map, save_attention_dir)
        #     self.vis_counter += 1  # カウンターをインクリメント
...
# define feature extraction & attention extraction model
class PretrainModel(nn.Module):
    def __init__(self, sarnn_model):
        super(PretrainModel, self).__init__()
        self.pos_encoder = sarnn_model.pos_encoder  # 事前学習したいパーツ
        self.im_encoder = sarnn_model.im_encoder  # 事前学習したいパーツ

    def forward(self, x):
        pos_output = self.pos_encoder(x)
        im_output = self.im_encoder(x)
        return pos_output, im_output

    # --- ▼▼▼【変更箇所】▼▼▼ ---
    def _save_attention_map(self, attn_map_tensor, save_path):
        """
        Attention Mapをヒートマップ画像として保存する内部ヘルパー関数。
        """
