...
class DiffullBPTTtrainer:
    ...
    def process_epoch(self, data, training=True):
        ...
        for n_batch, ((x_img, x_joint), (y_img, y_joint)) in enumerate(data):
            ...
            # 損失計算
            joint_dim = 7
            y_joint_sliced = y_joint[:, :, :joint_dim]  # カメラ座標系座標(7,8,9)を取り除く
            joint_target_diff = y_joint_sliced[:, 1:] - y_joint_sliced[:, :-1]

            img_loss = nn.MSELoss()(yi_hat, y_img[:, 1:]) * self.loss_weights[0]
            joint_loss = nn.MSELoss()(yv_hat, joint_target_diff) * self.loss_weights[1]
            pt_loss = nn.MSELoss()(
