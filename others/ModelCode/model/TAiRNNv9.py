...
class TAiRNNv9(nn.Module):
    ...
    def __init__(
        ...
        # Decoders
        input_dim = rec_dim + k_dim * 2
        self.decoder_pos = nn.Sequential(
            nn.Linear(input_dim, rec_dim // 2),
            activation,
            nn.Dropout(0.1),
    ...
    def forward(self, xi, xv, state=None):
        ...
        # 状態推定
        pos_input = torch.cat([h, dec_pts], dim=1)
        y_pos = self.decoder_pos(pos_input)
        logit_g = self.decoder_g(pos_input)  # (B, 1)
